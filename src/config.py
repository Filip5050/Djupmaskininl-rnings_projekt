...
class Config:
    ...
    # No categorical columns (all numerical after PCA)
    CATEGORICAL_COLS = []
    
    # Model hyperparameters for binary classification
    LAYER_SIZES = [128, 64, 32, 16]  # 4-layer deeper network for better feature learning
    ...
    TEST_SIZE = 0.2
    RANDOM_STATE = 42
    BATCH_SIZE = 256  # Larger batch for fraud detection
    EPOCHS = 50  # Increased from 30 - model stopped early at 22
    
    # Fraud detection specific
    USE_SMOTE = False  # DISABLED: SMOTE creates unrealistic patterns
